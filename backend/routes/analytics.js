...
// 記錄用戶行為
router.post('/track', validateRequest(schemas.trackEvent), async (req, res) => {
  try {
...
// 獲取用戶使用統計
router.get('/usage', validateQuery(querySchemas.usageQuery), async (req, res) => {
  try {
...
// 獲取功能使用趨勢
router.get('/trends', validateQuery(querySchemas.trendsQuery), async (req, res) => {
  try {
...
// 獲取性能指標
router.get('/performance', validateQuery(querySchemas.performanceQuery), async (req, res) => {
  try {
...
// 獲取錯誤統計
router.get('/errors', validateQuery(querySchemas.errorsQuery), async (req, res) => {
  try {
    const { period = '7d', severity, type } = req.validatedQuery;
    const userId = req.user.id;
...
// 獲取用戶行為分析
router.get('/behavior', validateQuery(querySchemas.behaviorQuery), async (req, res) => {
  try {
    const { period = '30d', action, page } = req.validatedQuery;
    const userId = req.user.id;
...
// 獲取轉換漏斗
router.get('/funnel', validateQuery(querySchemas.funnelQuery), async (req, res) => {
  try {
    const { steps, period = '30d', groupBy } = req.validatedQuery;
    const userId = req.user.id;
...
// 獲取留存分析
router.get('/retention', validateQuery(querySchemas.retentionQuery), async (req, res) => {
  try {
    const { period = '30d', cohort, interval = 'day' } = req.validatedQuery;
    const userId = req.user.id;
...
// 獲取熱門功能排行
router.get('/popular', validateQuery(querySchemas.popularQuery), async (req, res) => {
  try {
    const { period = '7d', limit = 10, category } = req.validatedQuery;
    const userId = req.user.id;
...
// 獲取實時監控數據
router.get('/realtime', validateQuery(querySchemas.realtimeQuery), async (req, res) => {
  try {
    const { metric, duration = '1h' } = req.validatedQuery;
    const userId = req.user.id;
...
// 導出分析報告
router.get('/export', validateQuery(querySchemas.exportAnalyticsQuery), async (req, res) => {
  try {
    const { type, period, format = 'json', filters } = req.validatedQuery;
    const userId = req.user.id;
...
// 設置分析偏好
router.put('/preferences', validateRequest(schemas.analyticsPreferences), async (req, res) => {
  try {
