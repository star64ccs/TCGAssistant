...
// 獲取用戶通知列表
router.get('/', validateQuery(querySchemas.notificationQuery), async (req, res, next) => {
  try {
...
// 發送通知
router.post('/send', validateRequest(schemas.sendNotification), async (req, res, next) => {
  try {
...
// 標記通知為已讀
router.put('/:notificationId/read', validateParams(paramSchemas.notificationIdParam), async (req, res, next) => {
  try {
...
// 批量標記通知為已讀
router.put('/batch-read', validateRequest(schemas.batchReadNotifications), async (req, res, next) => {
  try {
...
// 刪除通知
router.delete('/:notificationId', validateParams(paramSchemas.notificationIdParam), async (req, res, next) => {
  try {
...
// 更新通知設置
router.put('/settings', validateRequest(schemas.notificationSettings), async (req, res, next) => {
  try {
    const { settings,
    } = req.validatedBody;
    const userId = req.user.id;
...
// 訂閱推送通知
router.post('/subscribe', validateRequest(schemas.pushSubscription), async (req, res, next) => {
  try {
    const { endpoint, keys,
    } = req.validatedBody;
    const userId = req.user.id;
