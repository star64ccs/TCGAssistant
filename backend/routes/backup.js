...
// 創建備份
router.post('/create', validateRequest(schemas.createBackup), async (req, res, next) => {
  try {
...
// 獲取備份列表
router.get('/', validateQuery(querySchemas.backupQuery), async (req, res, next) => {
  try {
...
// 獲取備份詳情
router.get('/:backupId', validateParams(paramSchemas.backupIdParam), async (req, res, next) => {
  try {
...
// 下載備份
router.get('/:backupId/download', validateParams(paramSchemas.backupIdParam), async (req, res, next) => {
  try {
    const { backupId } = req.validatedParams;
    const userId = req.user.id;
...
// 恢復備份
router.post('/:backupId/restore', validateParams(paramSchemas.backupIdParam), validateRequest(schemas.restoreBackup), async (req, res, next) => {
  try {
    const { backupId,
    } = req.validatedParams;
...
// 刪除備份
router.delete('/:backupId', validateParams(paramSchemas.backupIdParam), async (req, res, next) => {
  try {
    const { backupId,
    } = req.validatedParams;
    const userId = req.user.id;
...
// 上傳備份文件
router.post('/upload', validateRequest(schemas.uploadBackup), async (req, res, next) => {
  try {
    const { backupData, description, type,
    } = req.validatedBody;
    const userId = req.user.id;
...
// 驗證備份完整性
router.post('/:backupId/verify', validateParams(paramSchemas.backupIdParam), async (req, res, next) => {
  try {
    const { backupId,
    } = req.validatedParams;
    const userId = req.user.id;
...
// 設置自動備份
router.put('/auto-backup', validateRequest(schemas.autoBackupSettings), async (req, res, next) => {
  try {
    const { enabled, frequency, retention, includeSettings, includeHistory, includeCollection,
    } = req.validatedBody;
    const userId = req.user.id;
...
// 獲取自動備份設置
router.get('/auto-backup', async (req, res, next) => {
  try {
    const userId = req.user.id;
...
// 手動觸發自動備份
router.post('/auto-backup/trigger', async (req, res, next) => {
  try {
    const userId = req.user.id;
...
// 批量操作備份
router.post('/batch-action', validateRequest(schemas.batchBackupAction), async (req, res, next) => {
  try {
    const { backupIds, action, data,
    } = req.validatedBody;
    const userId = req.user.id;
