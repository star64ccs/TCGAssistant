...
// 價格預測路由
router.get('/predict/:cardId', validateParams(paramSchemas.cardIdParam), validateQuery(querySchemas.priceQuery), async (req, res) => {
  try {
...
// 獲取歷史價格數據
router.get('/history/:cardId', validateParams(paramSchemas.cardIdParam), validateQuery(querySchemas.historyQuery), async (req, res) => {
  try {
    const { cardId } = req.validatedParams;
...
// 市場趨勢分析
router.get('/trends', validateQuery(querySchemas.priceQuery), async (req, res) => {
  try {
...
// 批量價格預測
router.post('/batch-predict', validateRequest(schemas.pricePrediction), async (req, res) => {
  try {
