...
// 獲取文件列表
router.get('/', validateQuery(querySchemas.fileQuery), async (req, res, next) => {
  try {
...
// 獲取文件詳情
router.get('/:fileId', validateParams(paramSchemas.fileIdParam), async (req, res, next) => {
  try {
...
// 下載文件
router.get('/:fileId/download', validateParams(paramSchemas.fileIdParam), async (req, res, next) => {
  try {
    const { fileId,
    } = req.validatedParams;
    const userId = req.user.id;
...
// 預覽文件
router.get('/:fileId/preview', validateParams(paramSchemas.fileIdParam), async (req, res, next) => {
  try {
    const { fileId,
    } = req.validatedParams;
    const userId = req.user.id;
...
// 更新文件信息
router.put('/:fileId', validateParams(paramSchemas.fileIdParam), validateRequest(schemas.updateFile), async (req, res, next) => {
  try {
    const { fileId,
    } = req.validatedParams;
...
// 刪除文件
router.delete('/:fileId', validateParams(paramSchemas.fileIdParam), async (req, res, next) => {
  try {
    const { fileId,
    } = req.validatedParams;
    const userId = req.user.id;
...
// 圖片處理
router.post('/:fileId/process', validateParams(paramSchemas.fileIdParam), validateRequest(schemas.imageProcessing), async (req, res, next) => {
  try {
    const { fileId,
    } = req.validatedParams;
...
// 批量操作文件
router.post('/batch-action', validateRequest(schemas.batchFileAction), async (req, res, next) => {
  try {
...
// 創建文件夾
router.post('/folder', validateRequest(schemas.createFolder), async (req, res, next) => {
  try {
    const { name, parentId, description,
    } = req.validatedBody;
    const userId = req.user.id;
...
// 獲取文件夾結構
router.get('/folder/structure', async (req, res, next) => {
  try {
    const userId = req.user.id;
...
// 移動文件
router.put('/:fileId/move', validateParams(paramSchemas.fileIdParam), validateRequest(schemas.moveFile), async (req, res, next) => {
  try {
    const { fileId,
    } = req.validatedParams;
...
// 複製文件
router.post('/:fileId/copy', validateParams(paramSchemas.fileIdParam), validateRequest(schemas.copyFile), async (req, res, next) => {
  try {
    const { fileId,
    } = req.validatedParams;
...
// 搜索文件
router.get('/search', validateQuery(querySchemas.fileSearchQuery), async (req, res, next) => {
  try {
    const { query, type, category, dateRange, sizeRange,
    } = req.validatedQuery;
    const userId = req.user.id;
