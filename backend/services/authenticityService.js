...
class AuthenticityService {
  ...
  // 真偽檢查
  async checkAuthenticity(cardId, cardType = 'pokemon', images = []) {
    try {
      logger.info(`開始真偽檢查: 卡片ID ${cardId}, 卡片類型 ${cardType}, 圖片數量 ${images.length}`);
      ...
      // 保存檢查歷史
      await this.saveCheckHistory(cardId, checkResult);
  ...
  // 詳細真偽分析
  async detailedAnalysis(cardId, cardType, analysisType = 'comprehensive', images = []) {
    try {
      logger.info(`詳細真偽分析: 卡片ID ${cardId}, 分析類型 ${analysisType}`);
      
      // 模擬詳細分析
      const detailedResults = {
        frontAnalysis: await this.analyzeCardFront(images),
        backAnalysis: await this.analyzeCardBack(images),
        edgeAnalysis: await this.analyzeCardEdges(images),
        textureAnalysis: await this.analyzeCardTexture(images)
      };
  ...
  async batchCheck(cards, images = []) {
    try {
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        const imageAnalysis = await this.analyzeImages(cardImages);
  ...
  // 獲取檢查歷史
  async getCheckHistory(userId, limit = 20, offset = 0) {
    try {
  ...
  calculateAuthenticityScore(imageAnalysis, cardType) {
    ...
    for (const [key, weight] of Object.entries(weights)) {
      totalScore += imageAnalysis[key] * weight;
    }
  ...
  generateIssues(imageAnalysis) {
    ...
    if (imageAnalysis.colorAccuracy < 85) {
      issues.push('顏色偏差');
    }
    if (imageAnalysis.edgeAnalysis < 75) {
      issues.push('邊緣處理不當');
    }
    if (imageAnalysis.hologramCheck < 70) {
      issues.push('全息圖案異常');
    }
  ...
  generateRecommendations(imageAnalysis, cardType) {
    ...
    if (imageAnalysis.colorAccuracy < 85) {
      recommendations.push('建議在不同光線條件下檢查顏色');
    }
    if (imageAnalysis.edgeAnalysis < 75) {
      recommendations.push('建議仔細檢查卡片邊緣');
    }
  ...
  // 私有方法：分析卡片質地
  async analyzeCardTexture(images) {
    return {
      score: Math.round((Math.random() * 30 + 70) * 100) / 100,
      issues: ['表面異常'],
  ...
  // 私有方法：保存檢查歷史
  async saveCheckHistory(cardId, checkResult) {
    try {
      ...
      logger.error('保存檢查歷史錯誤:', error);
    }
  }

  // 更新分析模型
  async updateAnalysisModel(cardType, newData) {
    try {
