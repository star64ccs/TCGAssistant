...
class FileManagerService {
  ...
  async uploadFiles(userId, files, options = {}) {
    try {
      const { category = 'general', description = '', tags = [],
      } = options;
      const uploadedFiles = [];
  ...
  // 獲取文件列表
  getFileList(userId, options = {}) {
    try {
      const { page = 1, limit = 20, category, type, search, sortBy = 'createdAt', sortOrder = 'desc',
      } = options;
  ...
  // 創建文件夾
  createFolder(userId, { name, parentId, description }) {
    try {
