...
const MultiSourceAutoUpdateSettingsScreen = () => {
  ...
  const handleToggleDataSource = async (sourceKey, enabled) => {
    try {
      ...
      if (result.success) {
        ...
        Alert.alert(
          t('common.success'),
          `${t('auto_update.source')
          } ${ enabled ? t('auto_update.enabled') : t('auto_update.disabled') }`,
        );
      } else {
  ...
  const renderDataSourceItem = ({ item }) => {
    const { type, key, name, enabled, status, lastUpdate, updateInterval, priority,
    } = item;
    return (
  ...
  const renderHistoryItem = ({ item }) => {
    const { startTime, endTime, summary, error,
    } = item;
    return (
  ...
  return (
    ...
    >
      ...
      <View style={ [styles.section, { backgroundColor: colors.card }]}>
        ...
        {
          lastUpdate ? <View style={styles.settingItem
            }>
              <View style={ styles.settingInfo }>
                <Text style={ [styles.settingLabel, { color: colors.text }]}>
                  { t('auto_update.last_update') }
      ...
      <View style={ [styles.section, { backgroundColor: colors.card }]}>
        ...
        <FlatList
          data={
            Object.entries(dataSourceStatus).flatMap(([type, sources]) =>
      ...
      { /* 時間選擇器 */ }
      {
        showTimePicker ? <DateTimePicker
            value={new Date(`2000-01-01T${updateTime
            }:00`)}
            mode="time"
            is24Hour={ true }
            display="default"
      ...
      >
        <View style={ [styles.modalContainer, { backgroundColor: colors.background }]}>
          ...
          {
            selectedSource ? <View style={[styles.sourceSettings, { backgroundColor: colors.card,
            }]}>
                <View style={ styles.settingItem }>
                  <Text style={ [styles.settingLabel, { color: colors.text }]}>
...
const styles = StyleSheet.create({
  ...
  viewAllButton: { paddingVertical: 4 },
  viewAllText: {
    fontSize: 14,
    fontWeight: '600',
  },
