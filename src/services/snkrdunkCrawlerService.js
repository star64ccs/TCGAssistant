...
class SnkrdunkCrawlerService {
  constructor() {
    ...
    this.delayBetweenRequests = 3000; // 3秒延遲
  ...
  async init() {
    try {
      console.log('初始化SNKRDUNK爬蟲服務...');
      await this.checkRobotsTxt();
      await this.loadCachedData();
      console.log('SNKRDUNK爬蟲服務初始化完成');
      return true;
    } catch (error) {
      console.error('SNKRDUNK爬蟲服務初始化失敗:', error);
      return false;
    }
  }
  ...
  async checkRobotsTxt() {
    try {
      console.log('檢查SNKRDUNK robots.txt...');
      ...
      console.log('SNKRDUNK robots.txt解析完成');
      
      // 儲存robots.txt規則到快取
      await AsyncStorage.setItem('snkrdunk_robots_rules', JSON.stringify(this.robotsRules));
      
      return true;
    } catch (error) {
      console.error('檢查SNKRDUNK robots.txt失敗:', error);
      // 使用預設規則
      this.robotsRules = {
        allowed: true,
        crawlDelay: 3,
  ...
  async searchCard(cardInfo, options = {}) {
    ...
    try {
      ...
      const searchUrl = `${this.searchUrl}?q=${encodeURIComponent(searchQuery)}&sort=price_asc`;

      console.log(`搜索SNKRDUNK: ${searchQuery}`);
      ...
      console.error('SNKRDUNK搜索失敗:', error);
      return {
        success: false,
        error: error.message,
        platform: 'SNKRDUNK',
  ...
  parseSearchResults(html, maxResults) {
    ...
    try {
      ...
      // 查找商品項目
      const itemPattern = /<div[^>]*class="[^"]*item[^"]*"[^>]*>(.*?)<\/div>/gs;
      const titlePattern = /<h3[^>]*class="[^"]*title[^"]*"[^>]*>(.*?)<\/h3>/s;
      const pricePattern = /<span[^>]*class="[^"]*price[^"]*"[^>]*>¥([0-9,]+)<\/span>/s;
      ...
      console.error('解析SNKRDUNK搜索結果失敗:', error);
    }
  ...
  generateCacheKey(cardInfo) {
    return `snkrdunk_${cardInfo.name}_${cardInfo.series || ''}_${cardInfo.cardNumber || ''}`;
  }

  // 載入快取數據
  async loadCachedData() {
    try {
      const cached = await AsyncStorage.getItem('snkrdunk_crawler_cache');
      if (cached) {
      ...
      console.error('載入SNKRDUNK快取數據失敗:', error);
    }
  }
  ...
  async saveCachedData() {
    try {
      ...
      await AsyncStorage.setItem('snkrdunk_crawler_cache', JSON.stringify(cacheArray));
    } catch (error) {
      console.error('儲存SNKRDUNK快取數據失敗:', error);
    }
  }
...
// 創建單例實例
const snkrdunkCrawlerService = new SnkrdunkCrawlerService();

export default snkrdunkCrawlerService;
